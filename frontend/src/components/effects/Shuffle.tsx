  ...
  useGSAP(
    () => {
      ...
      const build = () => {
        ...
        // Native DOM character splitter bypasses premium GSAP SplitText dependency cleanly
        const contentText = el.textContent || '';
        el.innerHTML = '';
        const chars: HTMLSpanElement[] = [];
        contentText.split('').forEach(char => {
      ...
      const inners = () => wrappersRef.current.map(w => w.firstElementChild as HTMLSpanElement);
      ...
      const cleanupToStill = () => {
        wrappersRef.current.forEach(w => {
          const strip = w.firstElementChild as HTMLSpanElement;
          if (!strip) return;
          const real = strip.querySelector('[data-orig="1"]') as HTMLSpanElement;
          if (!real) return;
